...
CompoundMailer.prototype.connect = function() {
    ...
    var type = this.config.driver;
    var ctopts;

    if(type.toLowerCase() != "smtp") {
        var transport = require("nodemailer-" + type.toLowerCase() + "-transport");
        ctopts = transport(opts);
    } else {
        ctopts = opts;
    }

    this.transport = nodemailer.createTransport(ctopts);
...
CompoundMailer.prototype.getViewContext = function() {
    ...
    var pathTo = this.compound.map.pathTo;
    var paths = Object.keys(pathTo);
    var basepath = this.config.baseURL ? this.config.baseURL : "";
    // Until we have ECMASCRIPT6 proxy
    c.pathTo = {};
    paths.forEach(function(path) {
        c.pathTo[path] = function() {
            return basepath + pathTo[path].apply(pathTo, arguments);
        };
        c.pathTo[path].toString = function() {
            return c.pathTo[path]();
        }
    });
    return c;
};
