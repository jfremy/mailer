...
Mail.prototype.compose = function compose(opts, cb) {
    ...
    mail.render('.html', function(err, html) {
        ...
        opts.html = juice(html);

        if(getView(mail.mailer.compound, mail.view, '.text')) {
...
function getView(compound, name, format) {
    var views = compound.structure.views;
    var viewPath = 'mail/' + name + format;
...
Mail.prototype.render = function render(format, cb) {
    ...
    var view = getView(compound, this.view, format);
    if (!view) {
